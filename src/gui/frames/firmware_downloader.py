...
class OptionsMenu(customtkinter.CTkToplevel):
    def __init__(self, master):
        ...
        self.install_firmware_label = customtkinter.CTkLabel(
            self.firmware_frame, text="Install Firmware:"
        )
        self.install_firmware_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")

        self.install_from_zip_button = customtkinter.CTkButton(
            self.firmware_frame,
            text="Install Firmware from ZIP",
            command=master.install_from_zip_button_wrapper,
        )
        self.install_from_zip_button.grid(row=1, column=0, padx=10, pady=5, sticky="w")

        self.install_from_directory_button = customtkinter.CTkButton(
            self.firmware_frame,
            text="Install Firmware from Directory",
            command=master.start_firmware_installation_from_directory,
        )
        self.install_from_directory_button.grid(
            row=2, column=0, padx=10, pady=5, sticky="w"
        )

        self.install_keys_button = customtkinter.CTkButton(
            self.firmware_frame,
            text="Install keys from ZIP/.keys file",
            command=master.install_keys_button_wrapper,
        )
        ...
        self.delete_files_checkbutton = customtkinter.CTkCheckBox(
            self.options_frame,
            text="Delete files after installing",
            variable=master.delete_download,
        )
        ...
        self.emulator_label = customtkinter.CTkLabel(
            self.options_frame, text="Install files for:"
        )
        self.emulator_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")

        self.emulator_dropdown = customtkinter.CTkOptionMenu(
            self.options_frame,
            variable=master.emulator_choice,
            values=["Yuzu", "Ryujinx", "Both"],
        )
        self.emulator_dropdown.grid(row=1, column=1, padx=10, pady=5, sticky="w")

        # Chunk size option
        self.chunk_label = customtkinter.CTkLabel(
            self.options_frame, text="Choose chunk size:"
        )
        ...
        for i in range(10):
            value = 16384 * (2**i)
            label = (
                f"{str(int((value)/1024))} KB"
                if value < 1024 * 1024
                else f"{str(int((value)/1024/1024))} MB"
            )
            chunk_size_list.append(label)
        self.chunk_size_dropdown = customtkinter.CTkOptionMenu(
            self.options_frame, variable=master.chunk_size, values=chunk_size_list
        )
        self.chunk_size_dropdown.grid(row=2, column=1, padx=10, pady=5, sticky="w")

        # Fetch versions command
        self.fetch_versions_button = customtkinter.CTkButton(
            self.options_frame,
            text="Attempt version fetch",
            command=master.fetch_versions,
        )
        self.fetch_versions_button.grid(
...
class FirmwareDownloader(customtkinter.CTkFrame):
    ...
    def options_menu(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = OptionsMenu(
                self
    ...
    def on_closing(self):
        if self.firmware_installation_in_progress or self.key_installation_in_progress:
            if not messagebox.askyesno(
                "Confirmation",
                "Are you sure you want to quit? The download in progress will be stopped",
            ):
                return
    ...
    def display_both_versions(self):
        while self.fetched_versions < 2:
            if self.error_fetching_versions:
                ...
                if messagebox.askretrycancel(
                    "Error",
                    f"Error while fetching versions. Retry?\n\nFull Error: {self.error_encountered}",
                ):
                    ...
                    messagebox.showinfo(
                        "EmuTool",
                        "You will only be able to install firmware and keys through files that are already downloaded by clicking \nFile > Install Firmware/Keys from ZIP/.keys file at the tom.",
                    )
                    return
        ...
        for firmware_version in self.firmware_versions:
            ...
            firmware_version_number = (
                "".join(re.split(r"\(|\)|\[|\]", firmware_version_number)[::2])
            ).replace(" ", "")
        ...
        for key_version in self.key_versions:
            ...
            if key_version_number in firmware_versions_dict:
                ...
                version_label = customtkinter.CTkLabel(
                    self.both_versions_frame,
                    text=f"{version} - Latest" if count == 0 else version,
                )
                version_label.grid(row=count, column=0, pady=10, sticky="W")
                version_button = customtkinter.CTkButton(
                    self.both_versions_frame,
                    text="Download",
                    command=lambda links=links: self.start_installation(
                        links, mode="Both"
                    ),
                )
    ...
    def fetch_firmware_versions(self):
        url = b64decode(
            "aHR0cHM6Ly9kYXJ0aHN0ZXJuaWUubmV0L3N3aXRjaC1maXJtd2FyZXMv".encode("ascii")
        ).decode("ascii")
        try:
        ...
        if len(self.firmware_versions) > 0:
            ...
            messagebox.showerror(
                "Connection Error", "Could not fetch firmware versions"
            )
    ...
    def display_firmware_versions(self, versions):
        ...
        for i, (version, link) in enumerate(versions):
            version_label = customtkinter.CTkLabel(
                self.firmware_versions_frame,
                text=f"{version} - Latest" if i == 0 else version,
            )
            version_label.grid(row=i, column=0, pady=10, sticky="W")
            version_button = customtkinter.CTkButton(
                self.firmware_versions_frame,
                text="Download",
                command=lambda link=link: self.start_installation(
                    link, mode="Firmware"
                ),
            )
    ...
    def display_key_versions(self, versions):
        ...
        for i, (version, link) in enumerate(versions):
            ...
            version_button = customtkinter.CTkButton(
                self.key_versions_frame,
                text="Download",
                command=lambda link=link: self.start_installation(link, mode="Keys"),
            )
            version_button.grid(row=i, column=1, pady=10, sticky="E")
        self.fetched_versions += 1
    ...
    def start_installation(self, link, mode):
        ...
        if mode == "Both":
            if (
                self.key_installation_in_progress
                or self.firmware_installation_in_progress
            ):
                messagebox.showerror(
                    "Error",
                    "There is already a firmware or key installation in progress!",
                )
                return
            ...
            Thread(target=self.start_key_installation, args=(link,)).start()
        elif mode == "Firmware":
            ...
            Thread(target=self.start_firmware_installation, args=(link,)).start()
    ...
    def install_keys(self, emulator, keys, status_frame=None):
        ...
        dst_folder = (
            os.path.join(os.path.join(os.getenv("APPDATA"), emulator), "keys")
            if emulator == "Yuzu"
            else os.path.join(os.path.join(os.getenv("APPDATA"), emulator), "system")
        )
    ...
    def start_key_installation_custom(self, path_to_key=None, status_frame=None):
        if self.key_installation_in_progress:
            messagebox.showerror(
                "Error",
                "There is already a key installation in progress. Please cancel the current installation to continue",
            )
            return
        ...
        try:
            if self.emulator_choice.get() != "Both":
                self.install_keys(
                    self.emulator_choice.get(), path_to_extracted_key, status_frame
                )
            else:
    ...
    def extract_keys_from_custom_zip(self, zip_location, status_frame):
        self.temp_directory = os.path.join(
            os.getenv("TEMP"), "Emulator Manager Extracts"
        )
        with open(zip_location, "rb") as file:
            with ZipFile(file) as archive:
                return self.extract_keys_from_zip(
                    archive, self.temp_directory, status_frame
                )
    ...
    def install_from_zip_button_wrapper(self):
        Thread(target=self.start_firmware_installation_from_custom_zip).start()

    def start_firmware_installation_from_custom_zip(
        self, path_to_zip=None, status_frame=None
    ):
        if self.firmware_installation_in_progress:
            messagebox.showerror(
                "Error",
    ...
    def extract_firmware_from_zip(
        self, archive, install_directory, emulator, status_frame=None
    ):
                ...
                for data in response.iter_content(chunk_size=chunk_size):
                    if download_status_frame.cancel_download_raised:
                        ...
                        download_status_frame.cancel_download_button.configure(
                            text="Remove",
                            command=download_status_frame.remove_status_frame,
                        )
                        return None
