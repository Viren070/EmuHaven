...
from customtkinter import (BooleanVar, CTkButton, CTkEntry, CTkFont, CTkFrame,
                           CTkLabel, CTkSwitch, CTkRadioButton, CTkOptionMenu, StringVar)
...
class SettingModal(CTkFrame):
    ...
    def update_setting_value(self, *args):
        ...
        if self.setting_type == "path":
            try:
                value = Path(self.setting_var.get().strip())
            except Exception as error:
                messagebox.showerror(self.winfo_toplevel(), "Invalid Path", f"The path you entered is invalid.\n\n{error}")
                self.update_entry_widget()
            if not value.exists():
                messagebox.showerror(self.winfo_toplevel(), "Invalid Path", "The path you entered does not exist.")
                self.update_entry_widget()
                return
