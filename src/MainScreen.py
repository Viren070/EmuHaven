...
class MainScreen(customtkinter.CTk):    # create class 
    ...
    def create_widgets(self):
        ...
        self.navigation_frame_label = customtkinter.CTkLabel(self.navigation_frame, text= "Emulator Manager v0.7.0",
...
# create dolphin settings widgets          
    ...
    def update_dolphin_setting_with_explorer(self, entry_widget):
        ...
        if entry_id == '5':
            dolphin_zip = filedialog.askopenfilename(initialdir=os.path.dirname(self.dolphin_settings_dict[entry_id]['var'].get()) if os.path.exists(self.dolphin_settings_dict[entry_id]['var'].get()) else os.getcwd(), title="Dolphin ZIP", filetypes=[("Dolphin 5.0-xxxxx.zip", "*zip")])
    ...
    def update_yuzu_setting_with_explorer(self, entry_widget):
        ...
        if entry_id == '5':
            yuzu_installer = filedialog.askopenfilename(initialdir=os.path.dirname(self.yuzu_settings_dict[entry_id]['var'].get()) if os.path.exists(self.yuzu_settings_dict[entry_id]['var'].get()) else os.getcwd(), title="Select Yuzu Installer", filetypes=[("yuzu_install.exe", "*exe")])
            ...
            zip_archive = filedialog.askopenfilename(initialdir=os.path.dirname(self.yuzu_settings_dict[entry_id]['var'].get()) if os.path.exists(self.yuzu_settings_dict[entry_id]['var'].get()) else os.getcwd(), title="Select {} ZIP".format("Firmware" if entry_id=="6" else "Key"), filetypes=[("Firmware" if entry_id=="6" else "Keys", "*zip")])
