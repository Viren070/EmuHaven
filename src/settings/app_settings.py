...
class AppSettings:
    def __init__(self, master):
        ...
        VALID_COLOUR_THEMES = get_colour_themes(
            os.path.join(master.root_dir, "assets", "themes"))
...
def custom_theme_is_valid(theme):
    with open(theme, "r") as file:
        ...
        try:
            ...
            print(
                f"[ERROR] app_settings.custom_theme_is_valid: Returning False:  {error}")
            return False
...
def load_customtkinter_themes(theme_folder):
    ...
    if os.path.exists(os.path.join(os.getcwd(), "PORTABLE.txt")):
        path_to_settings = os.path.join(os.getcwd(), "portable", "config", "settings.json")
    else:
    ...
    if os.path.exists(path_to_settings):
        ...
        with open(path_to_settings, "r", encoding="utf-8") as file:
            try:
                ...
                appearance_mode = app_settings["appearance_mode"] if app_settings[
                    "appearance_mode"] in VALID_APPEARANCE_MODES else "dark"
                colour_theme = app_settings["colour_theme"]
                if os.path.exists(colour_theme) and colour_theme.endswith(".json"):
                    colour_theme = colour_theme if custom_theme_is_valid(
                        colour_theme) else "dark-blue"
                    ...
                    colour_theme = os.path.join(
                        theme_folder, colour_theme+".json")
